...
def test_profile_model_applies_prism_rules(tmp_path: Path) -> None:
    ...
    train_meta_learner(records, output_dir=checkpoint_dir, model=model, epochs=5, seed=0)
    ...
    assert all(layer.adjusted_score >= layer.raw_score * 1.49 for layer in v_proj_layers)
    assert all(layer.adjusted_score <= layer.raw_score * 0.51 for layer in o_proj_layers)
